...
class SelfEncryptor:
    ...
    def encrypt_file(self, input_path: Path, output_path: Optional[Path] = None) -> Path:
        ...
        with open(output_path, "wb") as f:
            ...
            f.write(struct.pack("B", self.VERSION))              # [2] Versão
    ...
    def decrypt_file(self, vault_path: Path, output_path: Optional[Path] = None) -> Path:
        ...
        with open(vault_path, "rb") as f:
            ...
            pub_len = struct.unpack("H", f.read(2))[0]  # [6] Tam chave pública PEM
            public_pem = f.read(pub_len)                # [7] Chave pública PEM

            key_len = struct.unpack("H", f.read(2))[0]  # [8] Tam chave AES envolvida
            encrypted_aes_key = f.read(key_len)         # [9] Chave AES envolvida

            sig_len = struct.unpack("H", f.read(2))[0]  # [10] Tam assinatura
            signature = f.read(sig_len)                 # [11] Assinatura RSA-PSS

            nonce = f.read(12)                          # [12] Nonce GCM
            tag = f.read(16)                            # [13] Tag GCM
            ciphertext = f.read()                      # [14] Ciphertext restante
        logger.debug(
        ...
        # === BLOCO: VALIDAR DESTINATÁRIO ===
        # Verificar se foi cifrado para nós (public PEM coincide)
        ...
        # === BLOCO: DESENVOLTURA DA CHAVE SIMÉTRICA ===
        # DESENVOLTURA: recuperar chave AES via RSA-OAEP (usa private_key)
        ...
        # === BLOCO: VERIFICAÇÃO DA ASSINATURA ===
        # Verificar assinatura (autenticidade + integridade assimétrica)
